<script lang="ts">
  ...
  function applyConfig(next?: AppConfig) {
    if (!next) return;
    if (['hud', 'slim', 'orb'].includes(next.widget)) mode = next.widget as Mode;
    previousMode = mode;
    locked = !!next.locked;
    if (next.startSource === 'wave') safeApi('/api/wave');
    if (next.startHidden || next.hideUi) mode = 'orb';
  }

  function openSettings() {
    previousMode = mode === 'source' ? previousMode : mode;
    showSettings = true;
  }

  function closeSettings() {
    showSettings = false;
    if (mode === 'source') mode = previousMode;
  }

  function connectWS() {
    const protocol = location.protocol === 'https:' ? 'wss' : 'ws';
    ws = new WebSocket(`${protocol}://${location.host}/ws/state`);
    ws.onmessage = (event) => { try { state = { ...state, ...JSON.parse(event.data) }; syncError = false; } catch (e) { console.error(e); } };
    ws.onclose = () => { syncError = true; reconnectTimer = window.setTimeout(connectWS, 1500); };
    ws.onerror = () => ws?.close();
  }
  ...
  onMount(() => {
    loadConfig();
    connectWS();
    const progressTimer = window.setInterval(() => { const target = Math.max(0, Math.min(1, Number(state.progress) || 0)); smoothProgress = smoothProgress + (target - smoothProgress) * 0.22; }, 80);
    return () => { window.clearInterval(progressTimer); if (reconnectTimer) window.clearTimeout(reconnectTimer); ws?.close(); };
  });
</script>
...
<div class="stage" style={`left:${x}px; top:${y}px`} class:locked class:desktop-pin={config?.desktopPin} class:topmost={config?.alwaysOnTop} on:mousedown={down}>
  {#if mode === 'hud'}

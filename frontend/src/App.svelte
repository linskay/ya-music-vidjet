<script lang="ts">
  ...
  async function api(path: string) {
    try { await fetch(path); } catch (e) { console.error(e); }
  }
...
<div class="stage" style={`left:${x}px; top:${y}px`} class:locked on:mousedown={down}>
  {#if mode === 'hud'}
    <section class="hud frame-glow">
      <div class="hud-edge edge-a"></div><div class="hud-edge edge-b"></div>
      ...
      <main class="hud-body">
        <div class="cover-wrap"><img src={track.cover} alt="cover" /><div class="equalizer"><i></i><i></i><i></i><i></i><i></i></div></div>
        <div class="info">
          <h1>{track.title}</h1>
          <h2>{track.artist}</h2>
          <div class="progress-row"><span>{track.current}</span><div class="progress"><b></b></div><span>{track.duration}</span></div>
          <div class="controls">
            <button class="hex" on:click|stopPropagation={() => api('/api/prev')}>◀◀</button>
            <button class="hex primary" on:click|stopPropagation={() => api('/api/play')}>Ⅱ</button>
            <button class="hex" on:click|stopPropagation={() => api('/api/next')}>▶▶</button>
            <button class="hex like" on:click|stopPropagation={() => api('/api/like')}>♡</button>
  ...
  {:else if mode === 'slim'}
    <section class="slim frame-glow">
      <img src={track.cover} alt="cover" /><div><b>{track.title}</b><span>{track.artist}</span></div>
      <button class="hex" on:click|stopPropagation={() => api('/api/prev')}>◀</button><button class="hex primary" on:click|stopPropagation={() => api('/api/play')}>Ⅱ</button><button class="hex" on:click|stopPropagation={() => api('/api/next')}>▶</button><button class="hex like" on:click|stopPropagation={() => api('/api/like')}>♡</button><button on:click|stopPropagation={() => mode='hud'}>HUD</button>
    </section>
  {:else if mode === 'source'}
  ...
  {:else}
    <button class="orb" on:click|stopPropagation={() => mode='hud'}><span></span></button>
  {/if}
</div>
...
<style>
  ...
  button{font:inherit;color:#7eefff;background:rgba(0,20,40,.38);border:1px solid #15d8ff;cursor:pointer;text-shadow:0 0 10px #35e8ff;}
  ...
  .orb{width:86px;height:86px;border-radius:50%;border-color:#e04cff;background:radial-gradient(circle,#331044,#050712 62%);box-shadow:0 0 30px #e04cff, inset 0 0 22px rgba(0,238,255,.2);animation:pulse 2.3s infinite}.orb span{display:block;width:34px;height:34px;margin:auto;border-radius:50%;background:#25eaff;box-shadow:0 0 24px #25eaff}

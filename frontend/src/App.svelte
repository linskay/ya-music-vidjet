<script lang="ts">
  ...
  onMount(() => {
    loadConfig();
    connectWS();
    window.setTimeout(() => booting = false, 950);
    let frame = 0;
    let lastTick = 0;
    ...
    const loop = (ts: number) => {
      ...
      if (ts - lastTick >= tickMs) {
        ...
        document.documentElement.style.setProperty('--react', Math.max(0, Math.min(1.4, reactive)).toString());
...
{#if booting}<div class="boot-screen"><div class="boot-logo">YM</div><div class="boot-text">YA MUSIC WIDGET</div><div class="boot-line"><span></span></div></div>{/if}

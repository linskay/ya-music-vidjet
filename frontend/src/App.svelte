<script lang="ts">
  ...
  function safeApi(path: string) {
    const now = Date.now();
    if (now - lastCall < 180) return;
    lastCall = now;
  ...
  function setVolume(value: number) {
    volume = value;
    safeApi(`/api/volume/${(volume / 100).toFixed(2)}`);
  }
...
<div class="stage" style={`left:${x}px; top:${y}px`} class:locked on:mousedown={down}>
  ...
  {:else if mode === 'slim'}
    <section class="slim frame-glow" class:paused={!state.playing}><img src={cover} alt="cover" /><div><b>{title}</b><span>{artist}</span></div><button class="hex" on:click|stopPropagation={() => safeApi('/api/prev')}>◀</button><button class="hex primary" on:click|stopPropagation={() => safeApi('/api/play')}>{playLabel}</button><button class="hex" on:click|stopPropagation={() => safeApi('/api/next')}>▶</button><button class="hex like" class:active={state.liked} on:click|stopPropagation={() => safeApi('/api/like')}>♡</button><button on:click|stopPropagation={() => mode='hud'}>HUD</button></section>
  {:else if mode === 'source'}
    <section class="source-menu frame-glow"><header>ИСТОЧНИК <button on:click|stopPropagation={() => mode='hud'}>×</button></header><button on:click|stopPropagation={() => { safeApi('/api/wave'); mode='hud'; }}>▥ МОЯ ВОЛНА</button><button>☷ ПЛЕЙЛИСТЫ</button><button>☆ НОВИНКИ</button><button>HQ КАЧЕСТВО <span>HQ</span></button></section>
  {:else}
